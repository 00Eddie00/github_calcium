...
def main():
    ...
    sidetype = 3
    boungridt_file = open("boungridt3.dat", "w+")
    bounpoch_file = open("bounpoch3.dat", "w+")
    # sidetype=1 离散形式的边界
    if sidetype == 1:
        filename = 'boun3.dat'
        loadboun(filename)
    ...
    # sidetype=2 单段连续边界，函数形式，须改动
        ...
        arc = 2 * pai
        arcscatter(x0, y0, x1, y1, arc)
        savedata(4, boungridt_file, bounpoch_file)  # npoch是什么
    # sidetype=3 分段连续边界，函数形式，须改动
        ...
        for iloop in range(0, 17):
            print("**********")
            x0 = xbo[iloop]  # 每段起点终点坐标
            y0 = ybo[iloop]
            x1 = xbo[iloop + 1]
            y1 = ybo[iloop + 1]
            arc = arcbo[iloop]  # 每个点的步长
            arcscatter(x0, y0, x1, y1, arc)
            np = np - 1
            print("iloop", iloop)
            print("np", np)
            print("pstep",pstep[:60])
...
def xyarcbo():
    ...
    arc = 0
    arcbo[0] = 15
    arcbo[1] = rad * (pai / 4) - 30  # 45° * 300 - 30
    arcbo[2] = 30
