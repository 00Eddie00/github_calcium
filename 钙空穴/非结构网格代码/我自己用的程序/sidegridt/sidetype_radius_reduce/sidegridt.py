...
def main():
    ...
    sidetype = 2
    boungridt_file = open("boungridt_small.dat", "w+")
    bounpoch_file = open("bounpoch_small.dat", "w+")
    # sidetype=1 离散形式的边界
    if sidetype == 1:
        filename = 'boun3.dat'
        loadboun(filename)
    ...
    # sidetype=2 单段连续边界，函数形式，须改动
        ...
        for iloop in range(0, 9):   # 遍历9段圆弧。 一共10个点，第一个点和最后一个点是同一个点
            ...
            np = np - 1        # 最后一个点并进来，下次循环的时候又会当作第一个点并进来，重复了，所以每一段弧终点删掉
...
def xyarcbo():
    ...
    npoch[7] = 1
    npoch[8] = 2
    for i in range(0, 10):              # 最后一个点和第一个点重合(225,0)，10个点, 9段弧长
...
def arcscatter(x0, y0, x1, y1, arc):
    ...
    while True:
        ...
        if (arcxy * 1.5) <= (arc0 - pstep[np-1]):     # arc0 - pstep[np-1] 就是上一个点的步长
            arc0 = arc0 - arcxy * 0.5
            # 可以理解为如果新加点的步长过小，那么上个点的步长可以小一点，那不加这个点，返回循环重新判断新加点
        else:
            np = np + 1
            px[np-1] = x     # 新加点的坐标
            py[np-1] = y
