...
def delaunay():
    ...
    bmax = 2000
    fm2 = 10 ** -8
    px = npp.zeros(pmax, float)  # x坐标，生成有100000个元素的一维浮点数组，数组元素以 0 来填充
    py = npp.zeros(pmax, float)  # y坐标，生成有100000个元素的一维浮点数组
    xmax = 0.0
    xmin = 0.0
    ymax = 0.0
    ymin = 0.0
    ...
    np = 0  # 点的个数
    ne = 0  # 三角形单元的个数
    ndel = 0  # ndel是外接圆包含这个点的单元数
    edel = npp.zeros(delmax, int)  # 引入一个新的点之后，看有多少个单元包含这个点，edel是被删掉的单元
...
def search_1st(p, start):  # 按照某个路径寻找第一个点
    ...
    for i in range(0, ne):
        ...
        if dcp < gcr:
            ending = current
            break
        epoch[current - 1] = nsearch  # 第current个三角形是找到的第nsearch个三角形
        dep = -1
        next = 0
        for j in range(0, 3):
            k = noe[j, current - 1]
...
def clear_up(p):  # 把该删除的单元都删除掉，把编号都更新，要么就是用新的单元来填充，要么就是把后面的单元号往前推
    ...
    for i in range(ne, nnew - 1):
        ...
        for j in range(i + 1, nnew):
            ...
            for k in range(0, 3):
                arr2[k] = nod[k, j]
            n1, n2, n3 = common_point(arr1, arr2)  # 从这开始就看不太懂
            if n3 != 2:
                continue
    ...
    for m in range(0, ne):
        for i in range(0, ne):
            ...
            for j in range(0, 3):
                if (nod[j, i] < 0):
                    flag = True
                    nod = npp.delete(nod, i, axis=1)  # axis：沿着它删除给定子数组的轴，如果未提供，则输入数组会被展开
...
def savedata():  # 把数据保存起来
    i = 0
    no1 = open("bgnod.dat", "w")
    no2 = open("bgnoe.dat", "w")
    for i in range(0, ne):
