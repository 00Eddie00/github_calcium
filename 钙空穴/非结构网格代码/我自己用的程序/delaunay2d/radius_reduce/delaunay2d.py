...
cx = npp.zeros(emax, float)  # 外接圆圆心坐标和半径
...
def main():
    ...
    # gx, gy为临时变量
    global ndel, edel, neibn
    # 引入一个新的点之后，看有多少个单元（背景网格？）包含这个点，ndel是外接圆包含这个点的单元数，edel是被删掉的单元
    global eos, side, nb, nps
    # neibn相邻单元的信息
    global newnp, ecrit, eop, bgeop, pstep
    # newnp代表新生成的节点数
    ...
    for i in range(0, ne):  # 判断每一个三角形单元是不是正常数组
        ...
        for j in range(0, 3):
            nei = noe[j, i]
            if nei == 0:  # 若该三角形的某条边包含外壳，则为不正常数组
                ecrit[i] = 1
                break
    background_grid()  # 背景网格，控制网格的密度
    print("background grid ok")
    bgsearch = 0
    for i in range(0, bgne):  # 为每一个点赋值一个属性，bgne背景网格点数
        bgepoch[i] = 0  # bgepoch背景网格的属性
    for i in range(0, np):  # np目前是边界点个数
        xx = px[i]
        yy = py[i]
...
def boun_grid():  # 把边界上的每一个点给引进来，作三角化，检查它的边界完整不完整，再把外部的四个点删除
    ...
    len_y = 0.0
    nps = 0
    sid = open("sidegridt.dat", "r")  # 读取边界点的信息，此为边界点的文件
    ...
    for line in sid.readlines():
        ...
        if sidcount == 0:
            ...
            if ymin > py[sidcount]:
                ymin = py[sidcount]
        sidcount = sidcount + 1  # 找出xy最大最小值及点的个数
    np = sidcount  # 382
    nps = np  # nps可能是边界点数
    sid.close()
    print("read ok")
    ...
    length = 1.2 * length
    convex_hull()  # 生成三角形外接圆圆心和半径
    ne = 2  # 三角形单元的个数
    lastnp = -1
    nsearch = 0
    for i in range(lastnp + 1, np):  # 把边界点连成网格，0~381
        if i == 500:
            print("234")
        j = search_1st(i + 1, ne)  # 搜寻第一个三角形单元，遵循某个路径
        search_all(i + 1, j)  # 以第一个找的三角形单元为基础，再把其他所有的都搜出来，从j开始
        if ndel >= 200:
            print("nodel=", ndel, "/i=", i)
            k = input()
...
def convex_hull():  # 先引入四个点把整个边界都给框起来，用凸多边形把边界包住
    ...
    i = 0
    j = 0
    ipx[0] = centx - 0.65 * length  # 外壳四个点的坐标
    ...
    for i in range(0, 2):
        for j in range(0, 3):
            gx[j] = ipx[-nod[j, i] - 1]  #外壳两个三角形每个点坐标
            gy[j] = ipy[-nod[j, i] - 1]
...
# ***********************************8
def search_1st(p, start):  # 按照某个路径寻找第一个点， 从start号三角形开始寻找
    '''o:ending'''
    global np, ne, nsearch, px, py, cx, cy, cr, nod, noe, epoch, ipx, ipy, centx, centy, length
    # np点的个数，ne三角形单元的个数， nsearch寻找到第几个单元，px, py代表x坐标，y坐标，ipx, ipy存放点的临时横纵坐标，临时变量，可能放圆的质心
    # nod是每个单元由哪几个点组成，noe每个单元三个相邻的单元是哪几个，# epoch是每个单元的属性，npoch是每个点的属性
    ...
    x = px[p - 1]
    y = py[p - 1]
    nsearch = nsearch + 1  # nsearch = 0 + 1
    ending = 0
    current = start  # start开始查找
    for i in range(0, ne):  # ne目前三角形单元个数
        if epoch[current - 1] == nsearch:  # 若该三角形读过
            break
        gcx = cx[current - 1]
        gcy = cy[current - 1]
        gcr = cr[current - 1]
        dcp = sqrt((x - gcx) ** 2 + (y - gcy) ** 2)  # 该点到圆心距离
        if dcp < gcr:
        ...
        for j in range(0, 3):  # 没找到
            ...
            if k < 1 or k == last or epoch[k - 1] == nsearch:  # 找到相邻且未被读过的单元
                continue
    ...
    current = ending
    ending = 0
    if current > 0:  # 找着一个三角形（包括自己）
        key = in_or_out(p, current)
        if key == -1:  # 在外接圆内但不在三角形内
            search_all(p, current)  # 传入点号和开始查找的背景网格单元三角形编号，查找所有的包含该点的三角形
            for i in range(0, ndel):
                j = edel[i]
                k = in_or_out(p, j)  # 判断点p是否在单元j内
                if k > -1:
                    ending = j
                    break
        else:  # 在三角形内
            ending = current  # 返回该三角形单元
    if ending == 0:  # 没找着三角形
        for i in range(ne - 1, -1, -1):  # 在所有的三角形中寻找包含该点的三角形
            gcx = cx[i]
            gcy = cy[i]
            gcr = cr[i]
...
def search_all(p, ending):  # 以刚才那个为基准找所有的
    ...
    num = 1
    while True:
        ele = edel[num - 1]  # 待删除单元编号
        for k in range(0, 3):
            nei = noe[k, ele - 1]  # 找到一个相邻的背景网格单元三角形
            if nei <= 0:
                continue
            gcx = cx[nei - 1]
            gcy = cy[nei - 1]
            gcr = cr[nei - 1]
            dcp = sqrt((x - gcx) ** 2 + (y - gcy) ** 2)
            if (dcp - gcr) < (fm2 * gcr):  # 在相邻的背景网格单元三角形的外接圆内
                add = 1
                for j in range(0, ndel):  # 判断该相邻三角形是否已经被添加过
                    if edel[j] == (nei):  # 若是，则取消添加
                        add = 0
                        break
                if add == 1:  # 添加一个待删除的三角形
                    ndel = ndel + 1
                    edel[ndel - 1] = nei
        num = num + 1
        if num > ndel:  # 找到全部外接圆包含该点的背景网格单元后退出
            break
...
def check_up(p):  # 找到所有单元之后，判断哪些单元要删除
    ...
    for j in range(0, ndel):
        epoch[edel[j] - 1] = -nsearch  # nsearch = 1
    ndel1 = ndel
    while True:
        flag = 0
        for j in range(0, ndel1):
            ele = edel[j]  # 待删除单元
            for k in range(0, 3):
                pp[k] = nod[k, ele - 1]  # 该单元三个点的编号
            for k in range(0, 3):
                nei = noe[k, ele - 1]  # 找到该单元无相邻三角形的边
                if (nei) > 0 and epoch[nei - 1] == -nsearch:
                    ...
                    if vol1 / vol2 < 10 ** -8:  # 该点在某条无相邻三角形的边上
                        flag = 1
                        break
            if flag == 1:  # 该点在某条无相邻三角形的边上，则取消删除该边
                epoch[ele - 1] = 0
                edel[j] = edel[ndel1 - 1]
                ndel1 = ndel1 - 1
                break
...
def clear_up(p):  # 把该删除的单元都删除掉，把编号都更新，要么就是用新的单元来填充，要么就是把后面的单元号往前推
    ...
    nnew = ne  # ne = 2
    for i in range(0, ndel):  # 找到待删除单元
        ele = edel[i]
        for j in range(0, 3):
            nei = noe[j, ele - 1]  # 待删除单元相邻的三角形
            if nei <= 0:  # 该边无相邻单元
                nnew = nnew + 1
                nod[j, nnew - 1] = p  # 加入该点，以待形成新单元
                for k in range(0, 3):
                    if k == j:
                        continue
                    nod[k, nnew - 1] = nod[k, ele - 1]  # 将剩余两点加入该单元，形成一个新单元
                noe[j, nnew - 1] = nei  # 新单元对应原单元的边为0（无相邻单元）
                neibn[nnew - ne - 1] = neibn[nnew - ne - 1] + 1  #neibn之前都是0
            elif epoch[nei - 1] != -nsearch:  # 该边有相邻单元，nsearch = 1
                nnew = nnew + 1
                nod[j, nnew - 1] = p
                for k in range(0, 3):
                    if k == j:
                ...
                for k in range(0, 3):
                    if noe[k, nei - 1] == ele:  # 如果该相邻的三角形的某条边相邻三角形是原三角形
                        noe[k, nei - 1] = nnew  # 则将其某条边的相邻三角形修改为新单元
                        break
    for i in range(ne, nnew - 1):  # 从新生成单元开始
        if neibn[i - ne] == 3:  # 看不懂
            continue
        for k in range(0, 3):
            arr1[k] = nod[k, i]  # 储存新生成单元的三个点的标号
        for j in range(i + 1, nnew):
            if neibn[j - ne] == 3:
                continue
            for k in range(0, 3):
                arr2[k] = nod[k, j]  # 储存另一个新生成单元的三个点的标号
            n1, n2, n3 = common_point(arr1, arr2)  # 比较两个新生成单元有无相同边
            if n3 != 2:  # 无共同边
                continue
            neibn[i - ne] = neibn[i - ne] + 1  # 看不懂
            neibn[j - ne] = neibn[j - ne] + 1
            noe[n1 - 1, i] = j + 1
            noe[n2 - 1, j] = i + 1
            if neibn[i - ne] == 3:
                break
    for i in range(0, nnew - ne):  # 还原临时变量
        neibn[i] = 0
        epoch[i + ne] = 0
        ecrit[i + ne] = 0

    fill = nnew  # 三角形数量
    for i in range(0, ndel):  # ndel待删除单元数量
        ele = edel[i]  # 待删除单元
        if ele > nnew - ndel:  # 待删除单元是新加入单元
            continue
        for j in range(0, 3):  # 用最后一个单元代替待删除单元
            nod[j, ele - 1] = nod[j, fill - 1]
            noe[j, ele - 1] = noe[j, fill - 1]
        ecrit[ele - 1] = ecrit[fill - 1]
        epoch[ele - 1] = epoch[fill - 1]
        for j in range(0, 3):
            nei = noe[j, ele - 1]  # 待删除单元某条边相邻的三角形
            if nei <= 0:
                continue
            for k in range(0, 3):
                if noe[k, nei - 1] == fill:  # 改变新单元编号
                    noe[k, nei - 1] = ele
        fill = fill - 1
        while True:
            if epoch[fill - 1] == -nsearch:
                ...
                break
        for j in range(0, 3):  # 找到待删除单元三个点的坐标
            k = nod[j, ele - 1]
            if k < 0:
                gx[j] = ipx[-k - 1]
                gy[j] = ipy[-k - 1]
                ...
                gx[j] = px[k - 1]
                gy[j] = py[k - 1]
        circumcircle()  # 计算待删除单元外接圆圆心坐标及半径
        cx[ele - 1] = gcx
        cy[ele - 1] = gcy
        cr[ele - 1] = gcr

    for i in range(ne + 1, nnew - ndel + 1):  # ne = 2, nnew - ndel + 1 看不懂
        ele = i
        for j in range(0, 3):
            k = nod[j, ele - 1]
            if k < 0:
...
def savedata():  # 把数据保存起来
    ...
    no1.write(str(ne) + "\n")  # 存入背景网格个数
    for i in range(0, ne):
        no1.write(str(nod[0, i]) + " ")  # 存入背景网格的三个点
        no1.write(str(nod[1, i]) + " ")
        no1.write(str(nod[2, i]) + "\n")
        no2.write(str(noe[0, i]) + " ")  # 存入背景网格的三个边相邻的三角形
        no2.write(str(noe[1, i]) + " ")
        no2.write(str(noe[2, i]) + "\n")
        ci.write(str(cx[i]) + " ")  # 存入背景网格的外接圆圆心及半径
        ci.write(str(cy[i]) + " ")
        ci.write(str(cr[i]) + "\n")
    no1.write(str(np) + "\n")  # 存入背景网格点的个数
    no1.close()
...
# ***************************************
def checkside():  # 检查边界完不完整，每一条边界都是三角形的一条边
    global np, ne, nsearch, px, py, cx, cy, cr, nod, noe, epoch, ipx, ipy, centx, centy, length
    # np点的个数，ne三角形单元的个数， nsearch寻找到第几个单元，px, py代表x坐标，y坐标，ipx, ipy存放点的临时横纵坐标，临时变量，可能放圆的质心
    # nod是每个单元由哪几个点组成，noe每个单元三个相邻的单元是哪几个，# epoch是每个单元的属性，npoch是每个点的属性
    global gx, gy, gcx, gcy, gcr, x, y, xce, yce
    ...
    n3 = 0
    vol = 0.0
    sid_file = open("sideface.dat", "r")
    sidconut = 0
    for line in sid_file.readlines():
        current_line = list(filter(not_empty, line.strip("\n").split(" ")))
        side[0, i] = current_line[0]  # 边界每一个边的起点编号
        side[1, i] = current_line[1]  # 边界每一个边的终点编号
        side[2, i] = current_line[2]  # 边界每一个边的属性（区分入流点、出流点和壁面点）
        i += 1
        sidconut = sidconut + 1
    nb = sidconut  # 统计小圆和大圆共有382个点
    sid_file.close()
    nsearch = 0
    for j in range(0, ne):  # 目前背景网格单元个数
        epoch[j] = 0
    start = 1
    for i in range(0, nb):
        if i == 444:
            print("123")
        k = ((i + 1) % 100) + 1  # k属于1到100
        p = np + k  # np = sidcount = 382
        pp1[0] = side[0, i]
        pp1[1] = side[1, i]
        pp1[2] = 0
        px[p - 1] = (px[side[0, i] - 1] + px[side[1, i] - 1]) / 2.0  # 找到该线段的中点坐标并且储存
        py[p - 1] = (py[side[0, i] - 1] + py[side[1, i] - 1]) / 2.0
        ending = search_1st(p, start)  # 中点坐标、从第一个三角形开始查找包含该点的第一个三角形
        pp2[0] = nod[0, ending - 1]
        pp2[1] = nod[1, ending - 1]
        pp2[2] = nod[2, ending - 1]
        n1, n2, n3 = common_point(pp1, pp2)  # 判断边界边与包含边界边中点的三角形有无共同边
        if n3 != 2:  # 无共同边
            for j in range(0, 3):  # 在包含边界边中点的三角形的三个相邻三角形中寻找有无共同边
                nei = noe[j, ending - 1]
                pp2[0] = nod[0, nei - 1]
                pp2[1] = nod[1, nei - 1]
                pp2[2] = nod[2, nei - 1]
                n1, n2, n3 = common_point(pp1, pp2)
                if n3 == 2 and n1 == 3:  # 若有共同边，则换三角形继续查找
                    ending = nei
                    break
        if n3 != 2:  # 若还是没有查到
            search_all(p, ending)  # 中点坐标、从第一个包含该点的三角形开始重新查找包含该点的第一个三角形
            for j in range(0, ndel):
                nei = edel[j]
                pp2[0] = nod[0, nei - 1]
                pp2[1] = nod[1, nei - 1]
                ...
                if n3 == 2 and n1 == 3:
                    ending = nei
                    break
        if n3 == 2 and n1 == 3:  # 有共同边
            start = ending  # 则从这个三角形开始查找
            vol = area(pp1[0], pp1[1], pp2[n2 - 1])  # n2是什么
            if vol < 0:
                eos[1, i] = ending  # 边界两侧的三角形编号，0存面积为正，1存面积为负
                eos[0, i] = noe[n2 - 1, ending - 1]
                j = noe[n2 - 1, ending - 1]  # j为相邻三角形
                pp2[0] = nod[0, j - 1]
                pp2[1] = nod[1, j - 1]
                pp2[2] = nod[2, j - 1]
                n1, n2, n3 = common_point(pp1, pp2)  # 原三角形重叠边所相邻的三角形与边界边的公共边
                vol = area(pp1[0], pp1[1], pp2[n2 - 1])  # 公共边与相邻三角形面积
...
def delete_outside():  # 在边界点引进来之后，把外面那四个点及其组成的单元全部删掉
    ...
    nei = 0
    flag = 0
    for i in range(0, nb):  # 统计小圆和大圆共有382个点
        e1 = eos[0, i]
        e2 = eos[1, i]
        flag = 0
        for j in range(0, 3):
            ...
            if nei != e2:
                continue
            noe[j, e1 - 1] = 0  # 相邻边为0，其余不变
            flag = flag + 1
        for j in range(0, 3):
            nei = noe[j, e2 - 1]
            if nei != e1:
                continue
            noe[j, e2 - 1] = 0  # 相邻边为0，其余不变
            flag = flag + 1
        if flag != 2:
            print("error")
    print("inside and outside separate finished!")
    for i in range(0, nb):  # 统计小圆和大圆共有382个点
        e2 = eos[1, i]  # 边界旁边面积为负的单元号
        nod[0, e2 - 1] = 0
        nod[1, e2 - 1] = 0
        nod[2, e2 - 1] = 0
        for j in range(0, 3):
            ...
            if nei == 0:
                continue
            nod[0, nei - 1] = 0  # 因为要删掉边界外的点，将其相邻的三角形的点也删除
            noe[j, e2 - 1] = 0  # 将其余两边都变为0
    e1 = 0
    while True:
        flag = 0
        for i in range(0, ne):  # 三角形单元个数
            if nod[0, i] != 0:  # 找到需要删除的三角形
                continue
            if nod[1, i] == 0:  # 说明该单元已删除，无需继续
                continue
            flag = flag + 1  # 找到一个就加一
            nod[1, i] = 0
            nod[2, i] = 0  # 删除该单元
            for j in range(0, 3):
                nei = noe[j, i]  # 找到被删除单元相邻单元
                if nei == 0:
                    continue
                nod[0, nei - 1] = 0  # 删除该点
                noe[j, i] = 0  # 删除一条边
        print(e1)
        e1 = e1 + 1  # 一个被删除的三角形及其周边待删除的三角形删除完毕
        if flag == 0:
            break
    print("delete ok, times=", e1)
    e1 = 0
    for i in range(0, ne):  # 三角形单元个数
        if nod[0, i] == 0:  # 该三角形被删除过
            continue
        e1 = e1 + 1
        if i + 1 == e1:
            continue
        for j in range(0, 3):  # 重新生成三角形的各种数组
            nod[j, e1 - 1] = nod[j, i]
            noe[j, e1 - 1] = noe[j, i]
            cx[e1 - 1] = cx[i]
            cy[e1 - 1] = cy[i]
        ...
        for j in range(0, 3):
            ...
            if nei == 0:
                continue
            for k in range(0, 3):  # 修改重新生成的三角形相邻三角形的数组
                if noe[k, nei - 1] - 1 == i:
                    noe[k, nei - 1] = e1
    ne = e1
...
def advance_front():  # 用前沿推进方法来生成节点
    ...
    f3 = 0.3
    nsearch = 0
    for i in range(0, ne):  # ne目前三角形个数
        epoch[i] = 0
    bgsearch = 0
    for i in range(0, bgne):  # bgne目前背景三角形个数
        bgepoch[i] = 0
    newnp = np  # np边界点个数
    for i in range(0, ne):
        if ecrit[i] != 1:  # 读过
        ...
        for j in range(0, 3):
            nei = noe[j, i]
            if nei > 0 and ecrit[nei - 1] != -1:  # 有相邻三角形且读过，找到前沿
                continue
            m = j + 2
            n = m + 1
            # ？？？
            ...
            xn = xn / lsp
            yn = yn / lsp
            lcs = (cx[i] - px[pp[n - 1] - 1]) * xn + (cy[i] - py[pp[n - 1] - 1]) * yn  # 在算什么
            sx[j] = cx[i] - lcs * xn  # 待插入点和其搜索半径
            sy[j] = cy[i] - lcs * yn
            ...
            if emx == emn:
                ...
                bgep, ss[j] = search_in_bg(sx[j], sy[j], emx, bgep, ss[j])  # ss[j]一开始为0
                # bgep:在哪个背景网格，ss[j]:该点步长
                if bgep == 0:  # 没找到该点属于哪个网格
...
# ****************************************************
# xx, yy为边界点坐标  start = 1  ep = 0  step = 0.0
def search_in_bg(xx, yy, start, result, step):  # 搜查这个点在背景网格哪个单元，start是从背景网格哪一个地方搜寻，因为有时候背景网格很复杂的话，搜寻也是讲究以定策略的
    ...
    v3 = 0.0
    v4 = 0.0
    bgsearch = bgsearch + 1  # bgsearch = 0 + 1
    current = start  # start = 1
    result = 0
    next = 0
    last = 0
    for i in range(0, bgne):
        if bgepoch[current - 1] == bgsearch:  # bgepoch[i] = 0表示该背景网格单元是否被读过  bgsearch = 1
            break
        gcx = bgcx[current - 1]
        gcy = bgcy[current - 1]
        gcr = bgcr[current - 1]
        dcp = sqrt((xx - gcx) ** 2 + (yy - gcy) ** 2)
        if (dcp - gcr) < gcr * fm2:  # 边界点在背景网格单元外接圆内
            result = current
            break
        bgepoch[current - 1] = bgsearch  # 表示该单元已读过
        dep = -1
        next = 0
        for j in range(0, 3):
            nei = bgnoe[j, current - 1]
            if nei < 1 or nei == last or bgepoch[nei - 1] == bgsearch:
            # 该背景网格单元无相邻三角形or该单元已被读过
                continue
            gcx = bgcx[nei - 1]
            gcy = bgcy[nei - 1]
            gcr = bgcr[nei - 1]
            dcp = sqrt((xx - gcx) ** 2 + (yy - gcy) ** 2)
            if (dcp - gcr) < gcr * fm2:  # 该点在相邻三角形外接圆内
                result = current
                break
            xce = 0
            yce = 0
            ...
            if dep < 0 or dep > dcp:  # 沿着相邻三角形继续寻找
                dep = dcp
                next = nei
        if next == 0:  # 无需继续沿着相邻三角形寻找
            break
        last = current
        current = next
    current = result  # 并非一开始的三角形或其相邻三角形
    result = 0
    if current > 0:  # 有包含该边界点的三角形
        for i in range(0, 3):
            for j in range(0, 3):
                arr[0, j] = 1
            ...
            arr[1, i] = xx
            arr[2, i] = yy
            bulk[i] = det(arr, 3)  # 分别计算该边界点与包含他的三角形三条边组成的三角形的面积
        ...
        if v3 < 10 ** -8:  # v1等于v2
            result = current
            step = 0
            for i in range(0, 3):  # 根据包含它的三个点的步长计算它的步长
                bulk[i] = bulk[i] / v1
                step = step + bgstep[bgnod[i, result - 1] - 1] * bulk[i]
        else:
            search_bg_all(xx, yy, current)  # 从第一个背景网格开始查找其余外接圆包含该边界点的三角形
            for i in range(0, ndel):
                e = edel[i]
                for k in range(0, 3):
                    for j in range(0, 3):
                    ...
                    arr[1, k] = xx
                    arr[2, k] = yy
                    bulk[k] = det(arr, 3)  # 分别计算该边界点与包含他的三角形三条边组成的三角形的面积
                v1 = bulk[0] + bulk[1] + bulk[2]
                v2 = abs(bulk[0]) + abs(bulk[1]) + abs(bulk[2])
                v4 = (v2 - v1) / v1
                if v4 < 10 ** -8:  # v1等于v2
                    result = e
                    step = 0
                    for k in range(0, 3):  # 根据包含它的三个点的步长计算它的步长
                        bulk[k] = bulk[k] / v1
                        step = step + bgstep[bgnod[k, result - 1] - 1] * bulk[k]
                    break

    if result == 0:  # 并非一开始的三角形或其相邻三角形
        for i in range(bgne - 1, -1, -1):  # 对所有背景单元进行查找
            gcx = bgcx[i]
            gcy = bgcy[i]
            gcr = bgcr[i]
            dcp = sqrt((xx - gcx) ** 2 + (yy - gcy) ** 2)
    ...
    # result是结果，如果在背景网格第五个单元里面，result就是5
...
def search_boun(xx, yy, result, step):  # 判断边界点是不是合理
    ...
    for i in range(bgne - 1, -1, -1):
        ...
        for k in range(0, 3):
            ...
            arr[1, k] = xx
            arr[2, k] = yy
            bulk[k] = det(arr, 3)  # 分别计算该背景网格三角形三条边和该点组成的三角形面积
        v1 = bulk[0] + bulk[1] + bulk[2]
        v2 = abs(bulk[0]) + abs(bulk[1]) + abs(bulk[2])
        v4 = (v2 - v1) / v1
        # if(v4<1e-4)then
        if vmin > v4:  # v1 < v2，即找到面积最小的三个三角形，即该点在该三角形内
            vmin = v4
            emin = i + 1
            vol[0] = bulk[0]
            vol[1] = bulk[1]
    ...
    for k in range(0, 3):
        ...
        step = step + bgstep[bgnod[k, emin - 1] - 1] * vol[k]  # 找到三角形，计算该点步长
    result = emin
    return result, step
...
def search_bg_all(xx, yy, ending):
    ...
    dcp = 0.0
    ndel = 1  # 包含该点的三角形个数
    edel[ndel - 1] = ending  # 包含该点的三角形数组
    num = 1
    bgepoch[ending - 1] = -bgsearch  # bgsearch = 1
    while True:
        ele = edel[num - 1]
        for k in range(0, 3):
            nei = bgnoe[k, ele - 1]
            if nei <= 0 or bgepoch[nei - 1] == -bgsearch:  # 该边无相邻三角形or该三角形已被读过
                continue
            gcx = bgcx[nei - 1]
            gcy = bgcy[nei - 1]
            gcr = bgcr[nei - 1]
            dcp = sqrt((xx - gcx) ** 2 + (yy - gcy) ** 2)
            if (dcp - gcr) < (fm2 * gcr):  # 将该相邻单元加入数组并标记已读
                ndel = ndel + 1
                edel[ndel - 1] = nei
