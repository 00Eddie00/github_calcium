...
def main():
    ...
    for i in range(0, np):
        ...
        if ep == 0:
            ...
            # 防止出错，ep为背景网格的某一个单元，ep为背景网格的单元号，等于0代表没有找到
            if ep == 0:
...
def boun_grid():  # 把边界上的每一个点给引进来，作三角化，检查它的边界完整不完整，再把外部的四个点删除
    ...
    for line in sid.readlines():
        ...
        px[sidcount] = current_line[0]  # 每个边界点对应的X坐标和y坐标
...
# ***********************************8
def search_1st(p, start):  # 按照某个路径寻找第一个点，(p, start)新点编号和所属的三角形编号。找到第一个包含这个点的单元
    '''o:ending'''
    global np, ne, nsearch, px, py, cx, cy, cr, nod, noe, epoch, ipx, ipy, centx, centy, length
    # np点的个数，ne三角形单元的个数， nsearch寻找到第几个单元，px, py代表x坐标，y坐标，ipx, ipy存放点的临时横纵坐标，临时变量，可能放圆的质心
    # nod是每个单元由哪几个点组成，noe每个单元三个相邻的单元是哪几个，# epoch是每个单元的属性，npoch是每个点的属性
...
def advance_front():  # 用前沿推进方法来生成节点
    ...
    for i in range(0, ne):
        ...
        for j in range(0, 3):
            ...
            if n > 3:
                n = n - 3
            ee[0] = bgeop[pp[m - 1] - 1]  # bgeop用来存放这个点在背景网格的哪一个单元里面
            ee[1] = bgeop[pp[n - 1] - 1]
            ...
            xn = xn / lsp  # 单位法向量
            yn = yn / lsp  # 单位法向量
            lcs = (cx[i] - px[pp[n - 1] - 1]) * xn + (cy[i] - py[pp[n - 1] - 1]) * yn  # cx cy为外接圆心坐标
            sx[j] = cx[i] - lcs * xn  # px_mid = (px1 + px2) / 2.0
            sy[j] = cy[i] - lcs * yn  # py_mid = (py1 + py2) / 2.0
            sr[j] = sqrt(cr[i] * cr[i] - lcs * lcs)  # l = npp.linalg.norm(npp.array([px1, py1]) - npp.array([px_mid, py_mid]))
            emx = max(ee[0], ee[1])  # 边界俩点所属的背景网格
            emn = min(ee[0], ee[1])
            if emx == emn:  # 所属同一个背景网格
                ss[j] = (pstep[pp[0] - 1] + pstep[pp[1] - 1]) / 2.0  #
            else:
                bgep, ss[j] = search_in_bg(sx[j], sy[j], emx, bgep, ss[j])
                if bgep == 0:  # 0没找到所属背景网格
            ...
            if lcs < gcr:  # 有节点位于新点与原前沿构成的三角形的外接圆内，则不要该点
                continue
            if bgep > 0:
                emx = bgep
            bgep, step = search_in_bg(xx, yy, emx, bgep, step)  # 新点背景网格编号和步长
            if bgep == 0:
                continue
            lcs = sqrt((xx - px[pp[j] - 1]) ** 2 + (yy - py[pp[j] - 1]) ** 2)
            if lcs < f1 * step:  # 步长过大，则不要该点
                continue
            p = newnp + 1  # 新点被加入
            if p > pmax:
            ...
            px[p - 1] = xx
            py[p - 1] = yy
            ep = search_1st(p, i + 1)  # ep找到的第一个包含这个点的单元
            if ep == 0:
                continue
            p2[0] = nod[0, ep - 1]
            p2[1] = nod[1, ep - 1]
            ...
            if p == 0:
                continue
            for k in range(0, 3):
                nei = noe[k, ep - 1]  # ep找到的第一个包含这个点的单元
                if nei != 0:  # 找到前沿边k
                    continue
                m = k + 2  # 前沿边的两个点
                n = m + 1
            ...
            if p == 0:
                continue
            search_all(p, ep)  # 引入一个新的点之后，看有多少个单元包含这个点，ndel是外接圆包含这个点的单元数，edel是被删掉的单元
            x1 = xx + step * f1
            x2 = xx - step * f1
            y1 = yy + step * f1
            y2 = yy - step * f1
...
def smooth():
    ...
    for i in range(0, ne):
        ...
        slmin = min(s1[0], s1[1], s1[2])  # 找到该三角形长度最短的边
        stmin = min(pstep[pp[0] - 1], pstep[pp[1] - 1], pstep[pp[2] - 1])  # 找到该三角形步长最小的点
        if cr[i] > slmin * f2 or cr[i] > stmin * f5:
            xx = cx[i]  # 外接圆圆心坐标
            yy = cy[i]
            bgep, step = search_in_bg(xx, yy, bgeop[pp[0] - 1], bgep, step)  # 外接圆圆心所在背景网格的编号及该点步长
            if bgep == 0:
                continue
            p = newnp + 1
            px[p - 1] = xx  # 将圆心加入点集
            py[p - 1] = yy
            j = min(i + 1, ne)
            ep = search_1st(p, j)  # 找到第一个包含外接圆圆心的单元
            if ep == 0:
                continue
            p2[0] = nod[0, ep - 1]
            p2[1] = nod[1, ep - 1]
            ...
            for k in range(0, 3):
                ...
                vx = vx / lcs
                vy = vy / lcs
                lcs = abs(vx * (xx - px[p2[n - 1] - 1]) + vy * (yy - py[p2[n - 1] - 1]))  #   出流边界的两个点中点到外接圆圆心的距离
                if lcs < f3 * step:
                    p = 0
                    break
            if p == 0:
                continue
            search_all(p, ep)  # 引入一个新的点之后，看有多少个单元包含这个点
            for j in range(0, ndel):
                for k in range(0, 3):
