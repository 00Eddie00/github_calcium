...
def main():
    ...
    # 存储数据
    gri = open("4RYRgridt.dat", "w")  # 把新生成的点写到这个文件中去
    for i in range(0, np):
        gri.write(str(px[i]) + " ")  # 每个点的横纵坐标
        gri.write(str(py[i]) + "\n")
    gri.close()
    bge = open("4RYRbgeop.dat", "w")  # 把每一个点在背景网格的哪一个单元也写到文件中去
    for i in range(0, np):
        bge.write(str(bgeop[i]) + "\n")
    bge.close()
    pst = open("4RYRpstep.dat", "w")  # 把每一个点所对应的步长也写到文件中去
    for i in range(0, np):
...
def boun_grid():  # 把边界上的每一个点给引进来，作三角化，检查它的边界完整不完整，再把外部的四个点删除
    ...
    len_y = 0.0
    nps = 0
    sid = open("4RYRsidegridt.dat", "r")  # 读取边界点的信息，此为边界点的文件
...
def search_all(p, ending):  # 以刚才那个为基准找所有的
    ...
    while True:
        ele = edel[num - 1]
        for k in range(0, 3):
            nei = noe[k, ele - 1]  # 查看已被删除单元的邻居单元是否包含此点
            if nei <= 0:
                continue
            gcx = cx[nei - 1]
            gcy = cy[nei - 1]
            ...
            if (dcp - gcr) < (fm2 * gcr):
                ...
                for j in range(0, ndel):
                    if edel[j] == (nei):  # 如果这个邻居是被删掉的单元，就不添加
                        add = 0
                        break
                if add == 1:
                    ndel = ndel + 1
...
def savedata():  # 把数据保存起来
    i = 0
    no1 = open("4RYRnod.dat", "w")
    no2 = open("4RYRnoe.dat", "w")
    ci = open("4RYRcir.dat", "w")
    no1.write(str(ne) + "\n")
    for i in range(0, ne):
...
def checkside():  # 检查边界玩不完整，每一条边界都是三角形的一条边
    ...
    n3 = 0
    vol = 0.0
    sid_file = open("4RYRsideface.dat", "r")
    sidconut = 0
