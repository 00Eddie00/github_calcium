...
def main():
    ...
    sidetype = 2
    boungridt_file = open("4RYRboungridt_small4.dat", "w+")
    bounpoch_file = open("4RYRbounpoch_small4.dat", "w+")
    # sidetype=1 离散形式的边界
    if sidetype == 1:
        filename = 'boun3.dat'
        loadboun(filename)
    ...
    # sidetype=2 单段连续边界，函数形式，须改动
        ...
        arc = 2 * pai * rad   # 小圆周长
        arcscatter(x0, y0, x1, y1, arc)
        savedata(2, boungridt_file, bounpoch_file)
...
def arc_to_xy(arc):
    ...
    rad = side_rad
    x = cent_x + rad * cos(arc / rad)  # L=θ*r,计算θ, x=rcosθ
    y = cent_y + rad * sin(- arc / rad)
    return x, y
...
def arcn_to_xy(arc):
    ...
    rad = 0.00
    rad = 300
    x = rad * cos((arc + arcboun) / rad)  # L=θ*r,计算θ, x=rcosθ
...
def arcscatter(x0, y0, x1, y1, arc):
    ...
    arc0 = 0.0  # 每到一个弧后重新置0，arc0相比pstep，要多一个步长，用作判断新加点合不合适
...
def background_grid():
    ...
    i = 0
    j = 0
    bgn = open("4RYRbgnod.dat", "r+")
    bgno = open("4RYRbgnoe.dat", "r+")
    bgg = open("4RYRbggridt.dat", "r+")
    bgs = open("4RYRbgstep.dat", "r+")
    bgncount = 0
    bggcount = 0
