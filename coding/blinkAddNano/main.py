...
from blink.blink import average_concentration, ca_current, blink_fn_equation, blink_gn_equation, cca_ryr_inside_store
from blink.constant import SAVE_PATH, NR, CCAJSR, CCAF, START_STEP, BCAF, BCAM, BTRC, BSRM, BSLM, KDCAF, KDCAM, KDTRC, \
    KDSRM, KDSLM, RELEASE_TIME, DT, END_TIME, SAVE_INTERVAL, CCACYTREST, DCARYR, KRYR2
from blink.grid_info import NP
...
def do_loop_diffusion():
    ...
    if START_STEP == 0:  # 开始前，先存储第0步初值
        ...
        current_step = int(s_data.iloc[(len(s_data) - 1), :].values[0])  # 数据结构是ndarray
