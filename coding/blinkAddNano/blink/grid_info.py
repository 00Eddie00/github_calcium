...
from blink.constant import GRID_PATH
...
grid = np.loadtxt(GRID_PATH + '/gridt.dat', dtype=np.float64)
...
nod = np.loadtxt(GRID_PATH + '/nod.dat', dtype=np.int64)
# NE为三角形单元的数量
NE = len(nod)
NOD = nod.T - 1
...
noe = np.loadtxt(GRID_PATH + '/noe.dat', dtype=np.int64)
NOE = noe.T

# npoch.dat为每个节点的属性：0代表内部节点，2代表入流边界或远场的节点，1代表壁面的节点，4代表出流边界节点
NPOCH = np.loadtxt(GRID_PATH + '/npoch.dat', dtype=np.int64)
