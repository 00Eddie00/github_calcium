...
import numpy as np
from blink.blink_coeff_matrix import LN_MATRIX, CTRL_AREA, NMAX, TRIANGLE_NUMBER, ABC_MATRIX, AREA, TOTAL_AREA
from blink.constant import B_INNER, B_INFLOW, B_OUTFLOW, K2, K1, DCAJSR, F, BCSQ, KDCSQ, DCAFSR, CCAFSR, DCAF, \
    DT, H_JSR, UNITEC, MOLNUM
from blink.grid_info import NOD, NPOCH, NP, PX, PY, NE
...
def ca_current(fn, DCARYR):
    ...
    ca_in = ca_in * DCAFSR * H_JSR  # ca_in应该乘一个体积，这里乘了一个面积
...
def blink_core(fn, gn, CCACYTO, d_ca_ryr_jsr):
    new_fn = blink_fn_equation(fn, gn, CCACYTO, d_ca_ryr_jsr, 10)
    new_gn = blink_gn_equation(fn, gn, 10)
    c_ca_jsr = cca_ryr_inside_store(new_fn)
    return new_fn, new_gn, c_ca_jsr