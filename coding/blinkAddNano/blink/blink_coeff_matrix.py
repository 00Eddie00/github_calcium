...
from blink.constant import TRIANGLE_PATH
from blink.grid_info import NP, NE, NOD, PX, PY
...
def search_triangle(nmax):
    ...
    for n in range(0, NP):  # 遍历所有的点
        ...
        # 先检查有没有文件
        # 1.没有文件->写
        if not os.path.exists(TRIANGLE_PATH + "/POINT" + str(n + 1) + ".dat"):

            with open(TRIANGLE_PATH + "/POINT" + str(n + 1) + ".dat", "w") as file_object:
        ...
        # 2.有文件->直接读取
            ...
            with open(TRIANGLE_PATH + "/POINT" + str(n + 1) + ".dat", "r") as file_object:
