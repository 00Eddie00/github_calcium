...
# 新开放区域
new_open_z1 = np.cumsum(np.arange(5, 71))[1:]
new_open_z2 = np.negative(np.flipud(new_open_z1))

new_open_r1 = np.linspace(0, 200, 11)
new_open_r2 = np.linspace(200, 250, 6)
new_open_r3 = np.linspace(250, 295, 10)
new_open_r = np.concatenate((new_open_r1[:-1], new_open_r2[:-1], new_open_r3))

new_open_coordinates1 = np.concatenate(
    (np.tile(new_open_z1, len(new_open_r))[:, np.newaxis], np.repeat(new_open_r, len(new_open_z1))[:, np.newaxis]), axis=1)

new_open_coordinates2 = np.concatenate(
    (np.tile(new_open_z2, len(new_open_r))[:, np.newaxis], np.repeat(new_open_r, len(new_open_z2))[:, np.newaxis]), axis=1)


# ********************************************得到整个网格的点坐标*******************************************************
grid_coordinates = np.concatenate((nano_coordinates, boundary_coordinates, open_coordinates, new_open_coordinates2, new_open_coordinates1))
np.savetxt("../config/new_grid/grid_coordinates.csv", grid_coordinates, fmt='%.1f', delimiter=",")
