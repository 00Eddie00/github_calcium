...
nano_r = np.concatenate((nano_r1[:-1], nano_r2[:-1], nano_r3[:-1], nano_r4))  # [:-1]表示不含有最后一个元素
...
# ********************************************开放区域*********************************************
# 绘制开放区域的z轴
open_z1 = np.insert(np.cumsum(np.arange(5, 71)), 0, 0)  # 正半轴，左闭右开 [0,5,11,......]
open_z2 = np.negative(np.flipud(open_z1[1:]))  # 负半轴 [......,-11,-5]
